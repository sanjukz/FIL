CREATE PROC [dbo].[KITMS_GetInvoiceDetailsForCorpOrder_local]   --11829,3                      
(                      
--DECLARE                      
...
(ISNULL(ETA.LocalPrice,0) /((CAST((VAT.Value) AS DECIMAL(18,2))/100)+1))
ELSE ISNULL(ETA.LocalPrice,0) END END)) AS PricePerTic,
ISNULL(CRD.ApprovedTickets,0) AS Quantity,
CASE RequestOrderType WHEN 1 THEN 0                       
ELSE ISNULL((ETA.LocalPrice * CRD.ApprovedTickets),0) END AS TotalPrice,
...
ELSE
ISNULL(ETA.LocalPrice*CRD.ApprovedTickets,0) +
...
CONVERT(DECIMAL(18,2),CASE RequestOrderType WHEN 1 THEN 0 ELSE                    
 ...
 (ISNULL(ETA.LocalPrice,0)) -(ISNULL(ETA.LocalPrice,0) /((Cast((REPLACE(VAT.Value,'%','') ) 
