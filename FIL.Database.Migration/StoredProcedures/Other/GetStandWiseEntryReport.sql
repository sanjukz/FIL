CREATE PROCEDURE [dbo].[GetStandWiseEntryReport]  --6687                                                                                                                        
(                                                            
--Declare                                                          
	@EventId BIGINT  = 6688                                                                                                                                              
)                                                          
...
BEGIN                                                          
BEGIN                                                          
DECLARE @tblValues TABLE  (VenueCatName VARCHAR(1000), EntryCount INT, PaidCount INT, CompCount INT,SRCitiZenCount INT,ChildCount INT)                                                          
...
AND BarcodeNumber IS NOT NULL AND MSTD.EntryStatus = 1 AND EntryCount is not null AND EntryDateTime is not null  And TicketTypeId Not In(2,3)                                         
)                                        
,0))                                                          
 AS PaidCount,                                    
 ( ISNULL(                                                           
...
WHERE MSTD.EventTicketDetailId = ETA.EventTicketDetailId AND T.TransactionStatusId=8                                        
AND T.Id NOT In(Select TransactionId from TransactionPaymentDetails WITH(NOLOCK) WHERE  RequestType='Complimentary')                                                                         
AND BarcodeNumber IS NOT NULL AND MSTD.EntryStatus = 1 AND EntryCount is not null AND EntryDateTime is not null  And TicketTypeId=2                                                    
)                                                          
,0))                                                          
 AS ChildCount
 ...
 INNER JOIN TicketCategories TC WITH                                                          
...
,0))                                                          
 ...
 END                                                        
 BEGIN                                                          
 SELECT VenueCatName AS TicketCategory, SUM(EntryCount) AS EntryCount,SUM(CompCount+SRCitiZenCount+ChildCount) as  CompCount,SUM(PaidCount) as PaidCount                                                                       
 FROM @tblValues Where EntryCount>0                                                                                      
 GROUP BY  VenueCatName  ORDER BY SUM(EntryCount) DESC     
 END  
 END 
