CREATE PROC [dbo].[BO_GetEventTicketCategory_Test] ---22423,5276                                                                                                                         
(                                                                                                
...
WHERE ETD.IsEnabled=1 AND ED.IsEnabled=1 and ED.Id=@EventId                                                                                                                        
AND VenueId = @EventVenue  and ETA.RemainingTicketForSale > 0 and ETA.Id Not In(98721)          
UNION              
SELECT ETA.Id AS VMCC_Id,--TC.Name as VenueCatName,                     
CASE  WHEN ISNULL(ETA.SRCitizenQty,0) <>0 THEN TC.Name+' (Senior Citizen Qty - ' + CONVERT(VARCHAR(50),ETA.SRCitizenQty) +')'                                                                                    
...
--1 AS IsSeason,                                                                                          
--100 AS SeasonPrice,0 AS SeasonDiscount ,     
'false' AS IsSeason, 0 AS SeasonPrice,0 AS SeasonDiscount,                                                                             
...
INNER JOIN Venues V WITH(NOLOCK) ON V.Id = ED.VenueId                                                         
INNER JOIN Cities C WITH(NOLOCK) ON C.Id = V.CityId                              
WHERE ETD.IsEnabled=1 AND ED.IsEnabled=1 and ED.Id=@EventId   and ETA.Id Not In(98721)                                                                                                
