CREATE PROCEDURE  [dbo].[BO_GetOrderConfirmation] --210230375                             
(                                          
 @EventTransId BIGINT    
)                                          
AS                                              
...
SELECT A.Id AS EventTransID,A.AltId as TranscationAltId,  
A.FirstName,A.LastName,A.PhoneCode,A.PhoneNumber,A.EmailId,  
ED.Name AS EventName, D.Name AS VenueCatName, B.TotalTickets AS NoOfTic,          
...
C.Name AS City_Name,1 AS PickedBy, 2 AS [DeliveryType], EDTD.Notes AS PickUpnotes, '' AS CourierNotes, '' AS PAHNotes,     
...
B.Id AS VMCCMT_Id,B.TicketTypeId,EDTD.DeliveryTypeId ,RC.PaymentType                          
...
Inner Join EventDeliveryTypeDetails EDTD  WITH(NOLOCK) ON EDTD.EventDetailId=ED.ID                       
...
WHERE A.Id=@EventTransId  AND B.TotalTickets > 0    
