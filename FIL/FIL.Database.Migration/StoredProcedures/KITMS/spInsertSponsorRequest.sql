...
WHERE A.Id IN (SELECT Id FROM MatchSeatTicketDetails WHERE MatchLayoutSectionSeatId IN(SELECT * FROM dbo.SplitString(@TicNumIds, ',')))   
AND B.SeatTypeId = 1 AND SeatStatusId=1 AND A.SponsorId IS NULL AND TransactionId IS NULL  
   
SELECT @ActualSeatCount =COUNT(*) FROM dbo.SplitString(@TicNumIds, ',')  
...
DECLARE @PriceTable TABLE (RowNo INT IDENTITY(1,1), Price DECIMAL(18,2))    
 ...
 SELECT Id FROM MatchSeatTicketDetails WHERE MatchLayoutSectionSeatId IN(SELECT * FROM dbo.SplitString(@TicNumIds, ','))  
