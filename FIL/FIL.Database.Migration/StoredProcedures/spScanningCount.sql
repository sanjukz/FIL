...
From   #Matchseatticketdetails A  
Inner Join EventTicketDetails ED With (NoLOCK) On A.EventTicketDetailId=ED.ID  
INNER JOIN TicketCategories C WITH (NOLOCK) On ED.TicketCategoryId=C.Id  
LEFT OUTER JOIN Transactions T WITH (NOLOCK) ON T.Id = TransactionId  
LEFT OUTER JOIN Channels Ch WITH (NOLOCK) ON Ch.Id = T.ChannelId                                                                                                
 GROUP BY  ISNULL(EntryGateName,1)  ORDER BY Count(*) DESC   
  
-- select 1, '9' AS EntryGateName, 7218  AS EntryCount  
-- UNION      
--select 2, '10B' AS EntryGateName, 5416 AS EntryCount    
--UNION    
--select 3, '5' AS EntryGateName, 4796 AS EntryCount      
--UNION  
--select 4, '1' AS EntryGateName, 2948 AS EntryCount      
--UNION  
--select 5, '8' AS EntryGateName, 443 AS EntryCount      
--UNION  
--select 6, '7' AS EntryGateName, 356 AS EntryCount      
--UNION  
--select 7, 'Exhibit Entry' AS EntryGateName, 130 AS EntryCount      
