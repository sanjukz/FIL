...
{
    public class PartialVoidCommandHandler : BaseCommandHandler<PartialVoidTransactionCommand>
    {
        ...
        public PartialVoidCommandHandler(ITransactionRepository transactionRepository, ITransactionDetailRepository transactionDetailRepository,
            IEventTicketAttributeRepository eventTicketAttributeRepository, IEventTicketDetailRepository eventTicketDetailRepository,
            IMatchSeatTicketDetailRepository matchSeatTicketDetailRepository, IVoidRequestDetailRepository voidRequestDetailRepository,
        ...
        protected override async Task Handle(PartialVoidTransactionCommand command)
        {
            try
            {
                ...
                if (matchSeatTicketDetails.Any())
                {
                    foreach (MatchSeatTicketDetail item in matchSeatTicketDetails)
                    {
                        ...
                        if (command.ExchangeTransactionId != null)
                        {
                            var ticketRefundDetails = new TicketRefundDetail
                            {
                                BarcodeNumber = command.Barcodes.First(),
                                TransactionId = transactions.Id,
                                ActionTypeId = command.ActionTypeId,
                                IsEnabled = true,
                                ExchangedId = command.ExchangeTransactionId,
                                ExchangedAmount = command.ExchangedAmount,
                                RefundedAmount = 0,
                                ModifiedBy = command.ModifiedBy,
                            };
                            _ticketRefundDetailRepository.Save(ticketRefundDetails);
                        }
                    }
                }
            }
