...
namespace FIL.Api.QueryHandlers.TMS
{
    public class SponsorQueryHandler : IQueryHandler<SponsorQuery, SponsorQueryResult>
    {
        ...
        public SponsorQueryResult Handle(SponsorQuery query)
        {
            ...
            {
                eventDetail = AutoMapper.Mapper.Map<EventDetail>(_eventDetailRepository.GetByAltId((Guid)query.EventDetailAltId));
                SponsorList = AutoMapper.Mapper.Map<List<Sponsor>>(_sponsorRepository.GetFilteredSponsor(eventDetail.Id).Where(w => w.IsEnabled));
            }
            if (query.AllocationType == AllocationType.Venue)
            {
                ...
                SponsorList = AutoMapper.Mapper.Map<List<Sponsor>>(_sponsorRepository.GetFilteredSponsorByEventDetailIds(eventDetailList.Select(s => s.Id).Distinct()).Where(w => w.IsEnabled));
            }
